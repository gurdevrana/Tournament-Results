...
def playerStandings():
    ...
    with get_cursor() as c:
        ...
        for row in results:
            winner_id=int(row[0])
            c.execute("SELECT COUNT(*) FROM matches where winner=%s;",(row[0],))
            count = c.fetchall()
            wins = count[0][0]
            c.execute("SELECT COUNT(*) FROM matches where loser=%s;", (row[0],))
            count = c.fetchall()
            loses=count[0][0]
            matches=wins+loses
            res.append((row[0],row[1],wins,matches))
    return sorted(res, key=lambda tup: (tup[2]))
